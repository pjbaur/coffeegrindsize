...
from coffeegrindsize.analysis.statistics import compute_particle_statistics, weighted_stddev
...
from coffeegrindsize.io.csv_io import build_clusters_dataframe, build_stats_dataframe, load_clusters_dataframe
from coffeegrindsize.models import Comparison
from coffeegrindsize.utils import lighter
...
class coffeegrindsize_GUI:
            ...
            if self.polygon_alpha is not None:
                ...
                if npoly > 1:
                    ...
                    for i in range(npoly-1):
                        ...
                        #Redraw line
                        self.image_canvas.create_line(line_x_start, line_y_start, line_x_end, line_y_end, fill="green")
    ...
    def threshold_image(self, event):
        ...
        result = threshold_image_data(self.img_source, threshold, self.polygon_alpha, self.polygon_beta)
        self.imdata = result["imdata"]
        self.background_median = result["background_median"]
        self.mask_threshold = result["mask_threshold"]
        self.mask_threshold_edge = result["mask_threshold_edge"]
        ...
        self.img_threshold = result["img_threshold"]
    ...
    def refresh_cluster_data(self):
        ...
        for i in range(self.nclusters):
            ...
            for point_index in range(xlist.size):

                #Count neighbors
                ineigh = np.where((np.abs(xlist - xlist[point_index]) <= 1) & (np.abs(ylist - ylist[point_index]) <= 1))
                ...
                cluster_map_display[xlist[point_index], ylist[point_index], 0] = 255
                cluster_map_display[xlist[point_index], ylist[point_index], 1] = 0
                cluster_map_display[xlist[point_index], ylist[point_index], 2] = 0
    ...
    def quick_cluster(self, xlist, ylist, xstart, ystart):
        return quick_cluster(xlist, ylist, xstart, ystart)
    ...
    def psd_hist_from_data(self, source, hist_color=[147, 36, 30], hist_label=None, bins_input=None, histtype="bar", ypos_errorbar=None, xerr_shift=0.):
        ...
        plt.errorbar(xpdf_shifted, ypdf, yerr=np.array([poisson_neg,poisson_pos]), marker=".", markersize=0, linestyle="", color=hist_color_fm_error, elinewidth=2, capsize=0, alpha=0.8, zorder=15)
        ...
        capsize = 3
        capthick = 2
        plt.errorbar(avg, ypos_errorbar, xerr=xerr, marker=marker, markersize=markersize*1.4, linestyle="", color="w", elinewidth=elinewidth+2, capsize=capsize+1, capthick=capthick+1, alpha=0.8, zorder=19)

        plt.errorbar(avg, ypos_errorbar, xerr=xerr, marker=marker, markersize=markersize, linestyle="", color=hist_color_fm, elinewidth=elinewidth, ecolor=self.lighter(hist_color_fm,0.3), markeredgewidth=1.5, markeredgecolor="k", capsize=capsize, capthick=capthick, zorder=20)
    ...
    def weighted_stddev(self, data, weights, frequency=False, unbiased=True):
        return weighted_stddev(data, weights, frequency=frequency, unbiased=unbiased)
    ...
    def update_statistics(self):
        ...
        stats = compute_particle_statistics(
            self.clusters_long_axis,
            self.clusters_short_axis,
            self.clusters_surface,
            self.clusters_volume,
            pixel_scale,
        )
    ...
    def load_data(self, event):
        ...
        dataframe = load_clusters_dataframe(csv_data_filename)
    ...
    def save_data(self, event):
        ...
        dataframe = build_clusters_dataframe(
            self.clusters_surface,
            self.clusters_roundness,
            self.clusters_short_axis,
            self.clusters_long_axis,
            self.clusters_volume,
            pixel_scale,
        ...
        stats_dataframe = build_stats_dataframe(
            float(self.diam_average_var.get()),
            float(self.diam_stddev_var.get()),
            float(self.surf_average_var.get()),
            float(self.surf_stddev_var.get()),
            float(self.eff_var.get()),
